  ...
  return (
    <section className="relative w-full min-h-[85vh] flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-5xl mx-auto px-6 py-20 flex flex-col items-center text-center">
        ...
        {/* Headline */}
        <h1 className="mb-6 text-6xl md:text-8xl font-black text-[#1a1a1a] tracking-tight leading-[1.1]">
          Find the <span className="font-serif italic font-medium text-[#1b6b3e]">best</span>
          <br />
          <span className="font-serif italic font-medium text-[#1b6b3e]">prices</span> in Butuan.
        </h1>
 
        {/* Tagline */}
        <p className="mb-12 text-lg md:text-xl font-medium text-gray-600 max-w-2xl leading-relaxed">
        ...
        <form onSubmit={handleSearch} className="w-full max-w-3xl relative group">
          <div className="absolute -inset-1.5 bg-green-600/10 rounded-[2.5rem] blur opacity-0 group-hover:opacity-100 transition duration-500" />
          <div className="relative flex items-center bg-white rounded-full p-2 shadow-2xl shadow-green-900/10 border border-gray-100 focus-within:ring-2 focus-within:ring-green-600/20 transition-all">
             ...
             <input 
                ...
                className="flex-1 bg-transparent border-none outline-none text-gray-800 font-medium placeholder:text-gray-400 py-4"
             />
             <button 
               type="submit"
               className="bg-[#007e41] hover:bg-[#006b37] text-white rounded-full px-8 py-4 flex items-center gap-2 font-bold transition-all shadow-lg shadow-green-900/20 active:scale-95"
             >
                <span>Search</span>
          ...
          <div className="flex flex-wrap gap-4 items-center">
            {popularTags.map((tag) => (
              <Link 
                ...
                className="text-xs font-black text-gray-900 hover:text-[#1b6b3e] transition-colors border-b-2 border-transparent hover:border-[#1b6b3e] pb-1"
              >
                {tag}
              </Link>
            ))}
      ...
      {insight && (
        <div className="absolute hidden lg:block right-[5%] top-1/2 -translate-y-1/2 z-20 w-72 animate-bounce-slow">
           <div className="bg-white rounded-3xl p-6 shadow-2xl border border-gray-100 flex flex-col gap-3 group hover:-translate-y-2 transition-transform cursor-default">
      ...
      {/* ── Live Prices Bar ─────────────────────────────────────────── */}
      <div className="absolute bottom-0 left-0 right-0 z-20 bg-white/80 backdrop-blur-md border-t border-gray-100 py-3 overflow-hidden">
              ...
              <>
                {tickerItems.map((item, idx) => (
                  <div key={idx} className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-black text-gray-900">₱{item.price.toFixed(2)} /{item.unit}</span>
                    {item.change !== null ? (
                      <div className={cn(
